...
export class ExerciseLogService implements OnDestroy {
  ...
  async addAlternateLog(workout: Workout, exerciseIndex: number, alternate: ExerciseTemplate): Promise<string> {
    ...
    const data = {
      ...
      targetReps: trackingType !== 'duration' ? alternate.recommendedReps : undefined,
      targetWeight: trackingType === 'reps' ? alternate.recommendedWeight : undefined,
      targetDuration: trackingType === 'duration' ? alternate.recommendedDuration : undefined,
